...
class Solution {
    fun buildTree(preorder: IntArray, inorder: IntArray): TreeNode? {
        ...
        val preorderLeft = preorder.toList().subList(1, mid + 1).toIntArray()
        val inorderLeft = inorder.toList().subList(0, mid).toIntArray()

        val preorderRight = preorder.toList().subList(mid + 1, preorder.size).toIntArray()
        val inorderRight = inorder.toList().subList(mid + 1, inorder.size).toIntArray()
